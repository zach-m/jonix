...
{
	public JonixUniqueExporter(JonixUnifier<H, P> context)
	{
		super(context);
	}
	...
	protected static class ComparableProduct<H, P> implements Comparable<ComparableProduct<H, P>>
	{
		...
		{
			int result = JonixUtil.compareArray(_id, o._id);
	...
	protected boolean onBeforeFile(String fileName, JonixStreamer streamer)
	{
		...
		if (idColumn == null)
			idColumn = unifier.getDefaultIdColumn();
	...
	protected void onAfterFileList(List<String> processedFileNames, JonixStreamer streamer)
	{
		...
		log("Exporting " + filteredProductsEx.size() + " products..");
