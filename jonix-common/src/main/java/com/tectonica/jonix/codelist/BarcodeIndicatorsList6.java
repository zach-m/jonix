...
/**
 * marker interface to assist in IDE navigation to code-list 6 (Barcode indicator &lt;p&gt;NOTE: Deprecated in Onix3)
 */
interface CodeList6 {
}
 ...
 * <p>
 * Description: Barcode indicator &lt;p&gt;NOTE: Deprecated in Onix3
 *
...
public enum BarcodeIndicatorsList6 implements OnixCodelist, CodeList6 {
    ...
    BarcodeIndicatorsList6(String code, String description) {
        this.code = code;
    ...
    private static volatile Map<String, BarcodeIndicatorsList6> map;

    private static Map<String, BarcodeIndicatorsList6> map() {
        Map<String, BarcodeIndicatorsList6> result = map;
        if (result == null) {
            synchronized (BarcodeIndicatorsList6.class) {
                result = map;
                if (result == null) {
                    result = new HashMap<>();
                    for (BarcodeIndicatorsList6 e : values()) {
                        result.put(e.code, e);
                    }
                    map = result;
                }
    ...
    public static BarcodeIndicatorsList6 byCode(String code) {
