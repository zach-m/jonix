...
/**
 * marker interface to assist in IDE navigation to code-list 78 (Product form detail &lt;p&gt;NOTE: Deprecated in
 * Onix3)
 */
 ...
 * <p>
 * Description: Product form detail &lt;p&gt;NOTE: Deprecated in Onix3
 *
...
public enum ProductFormDetailsList78 implements OnixCodelist, CodeList78 {
    ...
    ProductFormDetailsList78(String code, String description) {
        this.code = code;
    ...
    private static volatile Map<String, ProductFormDetailsList78> map;

    private static Map<String, ProductFormDetailsList78> map() {
        Map<String, ProductFormDetailsList78> result = map;
        if (result == null) {
            synchronized (ProductFormDetailsList78.class) {
                result = map;
                if (result == null) {
                    result = new HashMap<>();
                    for (ProductFormDetailsList78 e : values()) {
                        result.put(e.code, e);
                    }
                    map = result;
                }
    ...
    public static ProductFormDetailsList78 byCode(String code) {
