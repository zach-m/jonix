 ...
 * the ONIX records: <ul> <li> {@code onHeaderElement} - when an ONIX &lt;Header&gt; tag was parsed (once per ONIX
 * source) <li> {@code onProductElement} - when an ONIX &lt;Product&gt; tag was parsed </ul> The events are fired to an
 * <b>extractor</b> - some implementation of {@link JonixExtractor} (passed in the constructor), which does the actual
 * processing of the data when sources are being read by this streamer.
 * <p>
